...
$$\text{Итоговый балл (100)} = \underbrace{\text{Методология (25)} + \text{Глубина и новизна (20)} + \text{Продукт и Сервис (25)} + \text{Код и Воспроизводимость (12)} + \text{Презентация (8)}}_{\text{90\% — экспертная оценка инженерно-продуктового решения}} + \underbrace{\text{Метрика (10)}}_{\text{10\% — авточек}}$$
...
4. **Метрика и валидность сабмита (Раздел 5 — 10 баллов):**
   ...
   - Идеальный контроль ложных тревог на парах межени: $\text{Spec}_{\text{base}} = 1.0$ (ложное затопление $< 0.05\%$ площади AOI).
...
- Вес зеркал воды (`water_peak` и `water_pre`) — суммарно **40%**. Порог — 200 га.
- Вес контроля межени ($\text{Spec}_{\text{base}}$) — **15%**.

### 5.2. Стратегия безупречной межени ($\text{Spec}_{\text{base}} = 1.0$)
$$\text{доля} = \frac{\max(0, \text{flood}_{\text{pred}} - \text{flood}_{\text{true}})}{\text{площадь района}}$$
$$\text{Spec}_{\text{base}} = \operatorname{mean}\left(1 - \min\left(1, \frac{\text{доля}}{0.005}\right)\right)$$
...
4. Проверяет условие `flood_ha <= water_peak_ha <= AOI_area`.
...
| **3. Deep Learning** | Тайлинг $1024 \times 1024$, обучение SegFormer с Modality Dropout | Дни 3–4 | Веса модели, $\text{Score} \ge 0.90$, $\text{Spec}_{\text{base}} = 1.0$ |

...
$$\text{Spec}_{\text{base}} = \text{среднее по 3 парам от } \left(1 - \min\left(1;\, \frac{\text{доля}}{0.005}\right)\right)$$
*(Ложное затопление свыше 0.5% площади района полностью обнуляет компоненту \text{Spec}_{\text{base}}).*
...
## 16. Порядок допуска к защите

По результатам проверки технических критериев формируется отдельный рейтинг команд.  
Для каждого технического критерия используется итоговый балл по критерию: при оценке несколькими экспертами — среднее арифметическое их оценок; для автоматической проверки — рассчитанный балл.  

**Итоговый технический балл** равен сумме баллов по всем техническим критериям (максимум 50 баллов).  

> [!IMPORTANT]
> **К защите допускаются не более 25 команд с наибольшим итоговым техническим баллом.**  
> Отраслевые критерии при формировании рейтинга допуска к защите **не учитываются**.

...
4. `predictions/<pair_id>_flood.tif` — бинарная растровая GeoTIFF-маска затопления (`uint8`, strictly $\{0, 1\}$, EPSG:32652).

---

## 2. Метрика оценки соревнования

Итоговый показатель качества рассчитывается по официальному регламенту:
...
2. **Движок генерации сабмита (`src/postprocessing/submission.py`):**
   - Калибровка базового уровня межени: строгое обнуление паводка на 3 контрольных парах межени, гарантирующее максимальный $Spec\_base = 1.0$.
...
1. **Интерактивный Web-GIS дашборд (`src/service/static/`):**
   - Одностраничное приложение (SPA) на Leaflet с темной гидрологической картографической подложкой CartoDB Dark Matter.
